...
$(function() {
  ...
  if ($('body').hasClass('mapfull')) {
    ...
    // Create infowindow
    infowindow =
    new OpenLayers.Popup("infowindow", new OpenLayers.LonLat(0, 0), new OpenLayers.Size(261, 140), '', false, null,
      {title:'Brasil Institute of Marine Research',datasets:"128",occurreces:"21,328",species:"982",url:'/members/detail.html'});
    ...
    for (var x = -45; x <= 45; x += dx) {
      for (var y = -22.5; y <= 22.5; y += dy) {
        ...
        features.push(new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(px, py),
          {x: px, y: py, title: "Poland", url: "/countries/detail.html", datasets:"45", species:"234", occurrences:"123,292"}));
        features2.push(new OpenLayers.Feature.Vector(new OpenLayers.Geometry.Point(px, py),
          {x: px, y: py, title: "Brazilian Institute for Marine Research", url: "/members/detail.html", datasets:"45", species:"234", occurrences:"123,292"}));
    ...
    var dblclick = new OpenLayers.Handler.Click(this, {dblclick: function() {
      map.zoomIn()
    }, click: null }, {single: true, 'double': true, stopSingle: false, stopDouble: true});
    dblclick.setMap(map);
    dblclick.activate();
...
function generateCountryMarkers(features) {
  ...
  setTimeout(function() {
    countries.addFeatures(features)
  }, 500);
