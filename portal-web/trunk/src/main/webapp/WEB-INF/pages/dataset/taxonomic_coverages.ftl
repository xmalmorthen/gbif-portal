...
<#if (dataset.taxonomicCoverages?size>0)>
  ...
  <#list dataset.taxonomicCoverages as cov>
    <#if cov.description?has_content>
      <h3>General taxonomic coverage description</h3>
    ...
    <#if cov.coverages?has_content>
      ...
      <ul class="three_cols">
        ...
        <#list cov.coverages as innerCov>
          <li><a href="<@s.url value='/species/search?q=${innerCov.scientificName!""}'/>">${innerCov.scientificName!""}<a/></li>
          <#if innerCov_index==8>
            <#assign more=true/>
            <#break />
          </#if>
      ...
      <ul class="three_cols">
        <li>Common Name</li>
        <#list cov.coverages as innerCov>
          <li><a href="<@s.url value='/species/search?q=${innerCov.commonName!""}'/>">${innerCov.commonName!"N/A"}<a/></li>
          <#if innerCov_index==8>
            <#break />
          </#if>
        </#list>
