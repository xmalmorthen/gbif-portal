...
public class OValValidationInterceptor extends MethodFilterInterceptor {
  ...
  protected String doIntercept(ActionInvocation invocation) throws Exception {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Intercepted method: [{}]", methodName);
      LOG.debug("Name: [{}]", context);
      LOG.debug("Namespace: [{}]", invocation.getProxy().getNamespace());
      LOG.debug("Proxy: [{}]", invocation.getProxy().getActionName());
    }
    ...
    if (violations != null) {
      ...
      for (ConstraintViolation violation : violations) {
        ...
        if (isActionError(violation)) {
          ...
          // add an .error extension to the field name and display the generic error i18n message
          validatorContext.addFieldError(fieldName, fieldName.concat(".error"));
