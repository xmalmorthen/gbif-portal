...
var dataHistory = (function() {
  ...
  function initialize(_values, opt) {
    ...
    max = _.max(values);
    normalized_values = _.map(values, function(v) {
      return height - Math.round(v * height / max);
    });
  ...
  function drawLines() {
    ...
    _.each(normalized_values, function(value, index) {
      path += template({xpos:(stepWidth * index) + .5, xpos2: (stepWidth * index) + .5, ypos:value +
                                                                                             .5, old_ypos:previous_value +
                                                                                                          .5});
      previous_value = value;
    });

    path += template({xpos:(stepWidth * values.length) + .5, xpos2:(stepWidth * values.length) + .5, ypos:height +
                                                                                                          .5, old_ypos:previous_value +
                                                                                                                       .5});
...
function drawGraph(ob, data, opt) {
  ...
  for (var i = 0, ii = data.length; i < ii; i++) {
    ...
    blanket.push(container.rect(leftGutter + X * i, 0, X, height - bottomGutter)
...
function drawLabelPie(ob, r, percentage, label1, label2, size1) {
  ...
  function sector(cx, cy, r, startAngle, endAngle, params) {
    ...
    return raphael.path(["M", cx, cy, "L", x1, y1, "A", r, r, 0, +(endAngle - startAngle > 180), 0, x2, y2, "z"])
      .attr(params);
  }
