...
sub recv_portal {
  ...
  if ( req.url ~ "^/(dataset|occurrence|species|member|node/|network/|ipt|installation|publisher|developer|country|cfg|css|fonts|img|js|favicon|participation/list|infrastructure)" || (req.url ~ "^/user/(downloads|namelist|cancel)")) {
...
sub vcl_fetch {
  ...
  if ( req.url ~ "^/tile-server") {
    ...
    # cache static files for 10 days in varnish and for an hour in the client
